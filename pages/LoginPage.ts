...
export class LoginPage {
  ...
  constructor(page: Page) {
    ...
    // Login form
    this.loginUsernameInput = page.locator("form").nth(0).locator("input[name='username']")
    ...
    this.resetPasswordUsernameInput = page.locator("form").nth(1).locator("input[name='username']")
    this.resetPasswordSubmitButton = page.locator('form').nth(1).locator('button[type="submit"]')
  ...
  async submitResetPasswordForm(username: string): Promise<void> {
    ...
    await this.resetPasswordUsernameInput.fill(username)
    await this.resetPasswordSubmitButton.click()
  }

  /**
   * Verifies that a success message is shown after password reset request.
   */
  async verifyResetPasswordSuccessMessage(expectedMessage: string): Promise<void> {
   ...
   */
  async verifyResetPasswordWasCancelled(): Promise<void> {
